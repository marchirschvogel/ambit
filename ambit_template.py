...
def main():
    ...
    IO_PARAMS            = {"problem_type"          : "solid_flow0d", # solid, fluid, fluid_ale, flow0d, solid_flow0d, fluid_flow0d, fluid_ale_flow0d, solid_flow0d_multiscale_gandr, solid_constraint
                            "mesh_domain"           : basepath+"/input/blocks_domain.xdmf", # domain mesh file (in 3D: volumes, in 2D: surfaces)
                            "mesh_boundary"         : basepath+"/input/blocks_boundary.xdmf", # boundary mesh file (in 3D: surfaces, in 2D: edges)
                            "mesh_subboundary"      : basepath+"/input/blocks_subboundary.xdmf", # sub-boundary mesh file (in 3D: edges, in 2D: points)
                            "mesh_subsubboundary"   : basepath+"/input/blocks_subsubboundary.xdmf", # sub-sub-boundary mesh file (in 3D: points, in 2D: -)
                            "mesh_encoding"         : "ASCII", # OPTIONAL: type of encoding of your mesh file (ASCII or HDF5) (default: "ASCII")
    ...
    FEM_PARAMS           = {"order_disp"            : 1, # order of displacement interpolation (solid or ALE mechanics)
                            ...
                            "incompressibility"     : "no", # if we want to use a 2-field functional for pressure dofs (always applies for fluid, optional for solid mechanics): "no", "full", or "nearly"
                            "bulkmod"               : 100., # in case "incompressibility" is set to "nearly", this would be the bulk modulus (the smaller, the more deviation from perfect compressibility)
