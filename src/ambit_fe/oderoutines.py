...
class ode:
    def __init__(self, init=True, ode_par=False, comm=None):
        ...
        self.varmap, self.auxmap = (
            {},
            {},
        )  # maps for primary and auxiliary variables
    ...
    def lambdify_expressions(self):
        ...
        utilities.print_status(
            "ODE model: Calling lambdify for residual expressions...",
            self.comm,
            e=" ",
        )
        ...
        ts = time.time()
        utilities.print_status(
            "ODE model: Calling lambdify for stiffness expressions...",
            self.comm,
            e=" ",
        )
        ...
        for i in range(self.numdof):
            for j in range(self.numdof):
                if self.dK_[i][j] is not sp.S.Zero:
                    self.dK__[i][j] = sp.lambdify(
                        [self.x_, self.c_, self.t_, self.fnc_],
                        self.dK_[i][j],
                        "numpy",
                    )
                else:
    ...
    def write_initial(self, path, nm, varTc_old, varTc, auxTc_old, auxTc):
        if self.ode_parallel:
            varTc_old_arr, varTc_arr = (
                allgather_vec(varTc_old, self.comm),
                allgather_vec(varTc, self.comm),
            )
        else:
        ...
        if self.comm.rank == 0:
            ...
            for i in range(len(self.varmap)):
                f1.write(
                    ...
                    % (
                        ...
                        varTc_old_arr[list(self.varmap.values())[i]],
                    )
                )
                f2.write(
                    "%s %.16E\n"
                    % (
                        list(self.varmap.keys())[i] + "_0",
                        varTc_arr[list(self.varmap.values())[i]],
                    )
                )
            ...
            for i in range(len(self.auxmap)):
                f1.write(
                    "%s %.16E\n"
                    % (
                        list(self.auxmap.keys())[i] + "_0",
                        auxTc_old[list(self.auxmap.values())[i]],
                    )
                )
                f2.write(
                    "%s %.16E\n"
                    % (
                        list(self.auxmap.keys())[i] + "_0",
                        auxTc[list(self.auxmap.values())[i]],
                    )
                )
