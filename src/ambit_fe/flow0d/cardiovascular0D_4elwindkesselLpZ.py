    ...
    def print_to_screen(self, var, aux):
        ...
        for n in range(self.num_models):
            utilities.print_status(
                "Output of 0D model (4elwindkesselLpZ) " + str(n + 1) + ":",
                self.comm,
            )

            utilities.print_status(
                "{:<1s}{:<3s}{:<10.3f}".format(self.cname[n], " = ", aux[4 * n + 0]),
                self.comm,
            )

            utilities.print_status(
                "{:<1s}{:<3s}{:<10.3f}".format(self.vname[n], " = ", var_arr[4 * n + 0]),
                self.comm,
            )
            utilities.print_status(
                "{:<1s}{:<3s}{:<10.3f}".format("g", " = ", var_arr[4 * n + 1]),
                self.comm,
            )
            utilities.print_status(
                "{:<1s}{:<3s}{:<10.3f}".format("q", " = ", var_arr[4 * n + 2]),
                self.comm,
            )
