...
def print_problem(ptype, sname, comm, numdof):
    ...
    if ptype == "solid":
        print_status(
            "###################### Welcome to finite strain solid mechanics #######################",
            comm,
        )

    elif ptype == "fluid":
        print_status(
            "############### Welcome to incompressible Navier-Stokes fluid mechanics ###############",
            comm,
        )

    elif ptype == "ale":
        print_status(
            "############################## Welcome to ALE mechanics ###############################",
            comm,
        )
        ...
        print_status(
            "################# Welcome to multiscale growth and remodeling (G & R) #################",
            comm,
        )
        print_status(
            "############## Small time scale: Monolithic 3D-0D coupled solid-flow0d ################",
            comm,
        )
        print_status(
            "####################### Large time scale: Static solid G & R ##########################\n",
            comm,
        )
        return
        ...
        print_status(
            "############# Welcome to Lagrange multiplier constraint solid mechanics ###############",
            comm,
        )

    elif ptype == "fluid_constraint":
        print_status(
            "############# Welcome to Lagrange multiplier constraint fluid mechanics ###############",
            comm,
        )

    elif ptype == "fluid_ale_constraint":
        print_status(
            "########### Welcome to Lagrange multiplier constraint ALE fluid mechanics #############",
            comm,
        )

    elif ptype == "fluid_flow0d":
        print_status(
            "########## Welcome to monolithic coupling of 3D fluid mechanics and 0D flow ###########",
            comm,
        )

    elif ptype == "fluid_ale_flow0d":
        print_status(
            "######## Welcome to monolithic coupling of 3D ALE fluid mechanics and 0D flow #########",
            comm,
        )

    elif ptype == "flow0d":
        print_status(
            "######################### Welcome to lumped-parameter 0D flow #########################",
            comm,
        )

    elif ptype == "signet":
        print_status(
            "######################### Welcome to signalling network models ########################",
            comm,
        )
    ...
    if isinstance(numdof, list):
        print_status(
            "Number of degrees of freedom: %i + %i" % (numdof[0], numdof[1]),
            comm,
        )
    else:
...
def mat_params_to_dolfinx_constant(matparams, msh):
    ...
    for k1 in matparams.keys():
        for k2 in matparams[k1].keys():
            if isinstance(matparams[k1][k2], dict):
