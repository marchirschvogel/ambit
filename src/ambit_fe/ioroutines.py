...
class IO:
    ...
    def readin_mesh(self):
        ...
        # # Now collect the domain cells
        # if isinstance(self.domain_ids, list):
        #     self.cells_domain = self.mt_d.indices[np.isin(self.mt_d.values, self.domain_ids)]
        # else: # can only be a locator function otherwise
        #     locator = self.domain_ids.evaluate
        #     self.cells_domain = mesh.locate_entities(self.mesh, self.mesh.topology.dim, locator)
        # print(self.cells_domain)
        # exit()
                ...
                for res in pb.results_to_write:
                    if res == "displacement":
                        ...
                        if self.output_midpoint:
                            ...
                            u = pb.u
                        evals, _, _ = spectral_decomposition_3x3(pb.ki.E(u))
                        ...
                        if self.output_midpoint:
                            ...
                            theta_out = fem.Function(pb.V_out_scalar, name=theta_proj.name)
                        ...
                        if self.output_midpoint:
                            ...
                            tau_out = fem.Function(pb.V_out_scalar, name=tau_proj.name)
...
class IO_fluid(IO):
                ...
                for res in pb.results_to_write:
                    if res == "velocity":
                        if self.output_midpoint:
                            ...
                            v_out = fem.Function(pb.V_out_vector, name=v_proj.name)
                        ...
                        for n in range(pb.io.num_domains):
                            if self.output_midpoint:
                                ...
                                v, p, F, chi = pb.v, pb.p_[n], pb.alevar["Fale"], pb.phasevar["chi"]
                            stressfuncs.append(pb.ma[n].sigma(v, p, F=F, chi=chi))
                        cauchystress = project(
                            stressfuncs,
                            pb.Vd_tensor,
                            pb.dx,
    ...
    def readcheckpoint(self, pb, N_rest):
        ...
        vecs_to_read[pb.v] = "v"
        vecs_to_read[pb.v_old] = "v_old"
        vecs_to_read[pb.v_veryold] = "v_veryold" # for BDF2 scheme
        vecs_to_read[pb.a_old] = "a_old"
        vecs_to_read[pb.uf_old] = "uf_old"  # needed for ALE fluid / FSI / FrSI
        vecs_to_read[pb.uf_veryold] = "uf_veryold"  # for BDF2 scheme
    ...
    def writecheckpoint(self, pb, N):
        ...
        vecs_to_write[pb.v_old] = "v_old"
        vecs_to_write[pb.v_veryold] = "v_veryold" # for BDF2 scheme
        vecs_to_write[pb.a_old] = "a_old"
        vecs_to_write[pb.uf_old] = "uf_old"  # needed for ALE fluid / FSI / FrSI
        vecs_to_write[pb.uf_veryold] = "uf_veryold" # for BDF2 scheme
...
class IO_phasefield(IO):
                ...
                for res in pb.results_to_write:
                    if res == "phase":
                        if self.output_midpoint:
                            ...
                            phi_out = fem.Function(pb.V_out_scalar, name=phi_proj.name)
                        ...
                        if self.output_midpoint:
                            ...
                            mu_out = fem.Function(pb.V_out_scalar, name=mu_proj.name)
    ...
    def readcheckpoint(self, pb, N_rest):
        ...
        vecs_to_read[pb.phi_veryold] = "phi_veryold" # for BDF2 scheme
    ...
    def writecheckpoint(self, pb, N):
        ...
        vecs_to_write[pb.phi_veryold] = "phi_veryold"
...
class IO_fsi(IO_solid, IO_fluid, IO_ale):
    ...
    def create_integration_measures(self, msh, sids, fids, iids, qdeg):
        ...
        if isinstance(self.dom_solid, list):
            ...
            self.cells_solid = mesh.locate_entities(self.mesh, self.mesh.topology.dim, locator)
        ...
        if isinstance(self.dom_fluid, list):
            ...
            self.cells_fluid = mesh.locate_entities(self.mesh, self.mesh.topology.dim, locator)

        if isinstance(self.surf_interf, list):
            self.cells_interface = self.mt_b.indices[np.isin(self.mt_b.values, self.surf_interf)]
        else: # can only be a locator function otherwise
            locator = self.surf_interf.evaluate
            self.cells_interface = mesh.locate_entities(self.mesh, self.mesh.topology.dim-1, locator)
