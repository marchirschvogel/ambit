...
class FluidmechanicsProblem(problem_base):
    def __init__(
        ...
        if self.pbase.have_rom:
            ...
            self.xdtr_old_, self.xintr_old_, self.xintrpre_ = (
                self.a_old,
                self.uf_old,
                self.uf_pre,
            )
        ...
        if self.pbase.restart_step == 0:
            if bool(self.initial_fluid_pressure):
                for m in range(self.num_dupl):
                    ...
                    self.p_[m].x.petsc_vec.ghostUpdate(
                        addv=PETSc.InsertMode.INSERT,
                        mode=PETSc.ScatterMode.FORWARD,
                    )
                    self.p_old_[m].x.petsc_vec.set(val)
                    self.p_old_[m].x.petsc_vec.ghostUpdate(
                        addv=PETSc.InsertMode.INSERT,
                        mode=PETSc.ScatterMode.FORWARD,
                    )
                ...
                if bool(self.duplicate_mesh_domains):
                    for m, mp in enumerate(self.io.duplicate_mesh_domains):
                        self.io.readfunction(
                            self.p_[m],
                            self.prestress_from_file[1].replace("*", str(m + 1)),
                        )
                        self.io.readfunction(
                            self.p_old_[m],
                            self.prestress_from_file[1].replace("*", str(m + 1)),
    ...
    def set_variational_forms(self):
        ...
        if self.is_multiphase:
            # mid-point representation of phase field
        ...
        for n, M in enumerate(self.domain_ids):
            ...
            if self.is_multiphase:
                ...
                rho_old_ = self.phasevar["phi_old"] * self.rho1[n] + (1.0 - self.phasevar["phi_old"]) * self.rho2[n]
                rho_mid_ = self.phasevar["phi_mid"] * self.rho1[n] + (1.0 - self.phasevar["phi_mid"]) * self.rho2[n]
            else:
            ...
            self.deltaW_int_old += self.vf.deltaW_int(
                self.ma[n].sigma(self.v_old, self.p_old_[j], F=self.alevar["Fale_old"], phi=self.phasevar["phi_old"]),
                self.dx(M),
                F=self.alevar["Fale_old"],
            )
            self.deltaW_int_mid += self.vf.deltaW_int(
                self.ma[n].sigma(self.vel_mid, self.pf_mid_[j], F=self.alevar["Fale_mid"], phi=self.phasevar["phi_mid"]),
                self.dx(M),
                F=self.alevar["Fale_mid"],
            )
            ...
            for nm in range(len(self.bc_dict["membrane"])):
                if "active_stress" in self.bc_dict["membrane"][nm]["params"].keys():
                    ...
                    if "weight" in self.bc_dict["membrane"][nm]["params"]["active_stress"].keys():
                        ...
                        self.io.readfunction(
                            wact_func,
                            self.bc_dict["membrane"][nm]["params"]["active_stress"]["weight"],
                        )
                ...
                if "field" in self.bc_dict["membrane"][nm]["params"]["h0"].keys():
                    ...
                    self.io.readfunction(
                        h0_func,
                        self.bc_dict["membrane"][nm]["params"]["h0"]["field"],
                    )
        ...
        (
            w_neumann_prestr,
            self.deltaW_prestr_kin,
            self.deltaW_prestr_int,
        ...
        (
            self.weakform_p,
            self.weakform_lin_vp,
            self.weakform_lin_pv,
    ...
    def set_problem_residual_jacobian_forms(self, pre=False):
        ...
        if self.have_robin_valve_implicit:
            ...
            for i in range(self.num_valve_coupling_surf):
                self.dw_robin_valve_dz_form.append(
                    fem.form(
                        self.dw_robin_valve_dz[i],
                        entity_maps=self.io.entity_maps,
                    )
                )
    ...
    def set_problem_vector_matrix_structures(self, rom=None):
        ...
        if self.have_robin_valve_implicit:
            ...
            self.K_vz = PETSc.Mat().createAIJ(
                size=(
                    (locmatsize, matsize),
                    (PETSc.DECIDE, self.num_valve_coupling_surf),
                ),
                bsize=None,
    ...
    def assemble_stiffness(self, t, subsolver=None):
        ...
        if self.have_robin_valve_implicit:
            ...
            for i in range(len(self.col_ids)):
                ...
                self.K_vz.setValues(
                    self.dofs_coupling_v[i],
                    self.col_ids[i],
                    self.k_vz_subvec[i].array,
                    addv=PETSc.InsertMode.INSERT,
                )
    ...
    def evaluate_robin_valve(self, t, dp_):
        ...
        for m in range(len(self.bc_dict["robin_valve"])):
            ...
            if self.bc_dict["robin_valve"][m]["type"] == "dp":
                ...
                to, tc = (
                    self.bc_dict["robin_valve"][m]["to"],
                    self.bc_dict["robin_valve"][m]["tc"],
                )
                if to > tc:
                    if t < to and t >= tc:
                        self.beta_valve[m].interpolate(self.beta_valve_max_expr[m].evaluate)
    ...
    def evaluate_flux_monitor(self, qv_, q_, prnt=True):
        for m in range(len(self.bc_dict["flux_monitor"])):
            ...
            if prnt:
                utilities.print_status(
                    "Flux ID " + str(self.bc_dict["flux_monitor"][m]["id"]) + ": q = %.4e" % (qv_[m]),
                    self.comm,
                )
    ...
    def solve_initial_prestress(self):
        ...
        for N in range(1, self.pb.prestress_numstep + 1):
            ...
            if self.pb.prestress_kinetic != "none":
                ...
                self.pb.v_old.x.petsc_vec.ghostUpdate(
                    addv=PETSc.InsertMode.INSERT,
                    mode=PETSc.ScatterMode.FORWARD,
                )
