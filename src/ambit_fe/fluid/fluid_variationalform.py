...
class variationalform(variationalform_base):
    ...
    def deltaW_kin_navierstokes_transient(self, a, v, rho, ddomain, w=None, F=None, phi=None):
        ...
        if self.formulation == "nonconservative":
            ...
            return ufl.dot(rho_*a + ufl.div(rho_*ufl.outer(v, v)), self.var_v) * ddomain

        else:
            raise ValueError("Unknown fluid formulation! Choose either 'nonconservative' or 'conservative'.")

    def deltaW_kin_navierstokes_steady(self, v, rho, ddomain, F=None, phi=None):
        rho_ = self.get_density(rho, phi=phi)
        if self.formulation == "nonconservative":
            assert(phi is None)
    ...
    def deltaW_kin_stokes_transient(self, a, v, rho, ddomain, w=None, F=None, phi=None):
        rho_ = self.get_density(rho, phi=phi)
        return rho_ * ufl.dot(a, self.var_v) * ddomain
    ...
    def f_inert_strong_navierstokes_transient(self, a, v, rho, w=None, F=None, phi=None):
        rho_ = self.get_density(rho, phi=phi)
        if self.formulation == "nonconservative":
            assert(phi is None)
            return rho_ * (a + ufl.grad(v) * v)
        elif self.formulation == "conservative":
            return rho_*a + ufl.div(rho_*ufl.outer(v, v))
        else:
            raise ValueError("Unknown fluid formulation!")
    ...
    def f_inert_strong_stokes_transient(self, a, rho, phi=None):
        rho_ = self.get_density(rho, phi=phi)
        return rho_ * a
    ...
    def stab_pspg(self, var_p, res_v_strong, tau_pspg, rho, ddomain, F=None, phi=None):
        rho_ = self.get_density(rho, phi=phi)
        return (1.0 / rho_) * ufl.dot(tau_pspg * ufl.grad(var_p), res_v_strong) * ddomain

    def stab_lsic(self, v, tau_lsic, rho, ddomain, F=None, phi=None):
        rho_ = self.get_density(rho, phi=phi)
        return tau_lsic * ufl.div(self.var_v) * rho_ * self.res_p_strong(v, F=F) * ddomain

    # components of element-level Reynolds number - cf. Tezduyar and Osawa (2000)
    def re_c(self, rho, v, ddomain, w=None, F=None, phi=None):
        rho_ = self.get_density(rho, phi=phi)
        return rho_ * ufl.dot(ufl.grad(v) * v, self.var_v) * ddomain

    def re_ktilde(self, rho, v, ddomain, w=None, F=None, phi=None):
        rho_ = self.get_density(rho, phi=phi)
...
class variationalform_ale(variationalform):
    ...
    def deltaW_kin_navierstokes_transient(self, a, v, rho, ddomain, w=None, F=None, phi=None):
        ...
        if self.formulation == "nonconservative":
            ...
            return rho_ * ufl.dot(a + ufl.grad(v) * ufl.inv(F) * (v - w), self.var_v) * J * ddomain
    ...
    def deltaW_kin_navierstokes_steady(self, v, rho, ddomain, w=None, F=None, phi=None):
        rho_ = self.get_density(rho, phi=phi)
        J = ufl.det(F)
        if self.formulation == "nonconservative":
            assert(phi is None)
    ...
    def f_inert_strong_stokes_transient(self, a, v, rho, w=None, F=None, phi=None):
        rho_ = self.get_density(rho, phi=phi)
        if self.formulation == "nonconservative":
            assert(phi is None)
    ...
    def stab_supg(
        ...
        J = ufl.det(F)
        if symmetric:  # modification to make the effective stress symmetric - experimental, use with care...
            return ufl.dot(tau_supg * ufl.sym(ufl.grad(self.var_v) * ufl.inv(F)) * v, res_v_strong)*J*ddomain
        else:
            return ufl.dot(tau_supg * ufl.grad(self.var_v) * ufl.inv(F) * v, res_v_strong)* J* ddomain

    def stab_pspg(self, var_p, res_v_strong, tau_pspg, rho, ddomain, F=None, phi=None):
        rho_ = self.get_density(rho, phi=phi)
        J = ufl.det(F)
        return (1.0 / rho_) * ufl.dot(tau_pspg * ufl.inv(F).T * ufl.grad(var_p), res_v_strong) * J * ddomain

    def stab_lsic(self, v, tau_lsic, rho, ddomain, F=None, phi=None):
        rho_ = self.get_density(rho, phi=phi)
        J = ufl.det(F)
        return tau_lsic * ufl.inner(ufl.grad(self.var_v), ufl.inv(F).T) * rho_ * self.res_p_strong(v, F=F) * J * ddomain
