...
class solver_base:
    ...
    def print_final(self):
        ...
        utilities.print_status(
            "{:<55s}{:<1.4f}{:<2s}".format("Total solution time of all steps: ", self.wt, " s"),
            self.pb.comm,
        )
        utilities.print_status(
            "{:<55s}{:<1.4f}{:<2s}".format(
                "Average solution time per time step: ",
                self.wt / self.Nfinal,
                " s",
            ),
            self.pb.comm,
        ...
        utilities.print_status(
            "{:<55s}{:<1.4f}{:<2s}".format("Maximum solution time of a time step: ", max(self.wt_), " s"),
            self.pb.comm,
        )
        utilities.print_status(" ", self.pb.comm)
        utilities.print_status(
            "{:<55s}{:<1d}".format("Total number of nonlinear iterations: ", self.ni),
            self.pb.comm,
        )
        utilities.print_status(
            "{:<55s}{:<1.1f}".format(
                "Average number of nonlinear iterations per time step: ",
                self.ni / self.Nfinal,
            ),
            self.pb.comm,
        )
        utilities.print_status(
            "{:<55s}{:<1d}".format(
                "Maximum number of nonlinear iterations in a time step: ",
                max(self.ni_),
            ),
            self.pb.comm,
        )
        ...
        if self.solnln.solvetype[0] == "iterative":
            utilities.print_status(" ", self.pb.comm)
            utilities.print_status(
                "{:<55s}{:<1d}".format("Total number of linear iterations: ", self.li),
                self.pb.comm,
            )
            utilities.print_status(
                "{:<55s}{:<1.1f}".format(
                    "Average number of linear iterations per time step: ",
                    self.li / self.Nfinal,
                ),
                self.pb.comm,
            )
            utilities.print_status(
                "{:<55s}{:<1d}".format(
                    "Maximum number of linear iterations in a time step: ",
                    max(self.li_),
                ),
                self.pb.comm,
            )
            utilities.print_status(
                "{:<55s}{:<1.1f}".format(
                    "Average number of linear iterations per solve: ",
                    self.li / self.ni,
                ),
                self.pb.comm,
            )
            utilities.print_status(
                "{:<55s}{:<1d}".format(
                    "Maximum number of linear iterations in a solve: ",
                    max(self.solnln.li_s),
                ),
                self.pb.comm,
            )
    ...
    def update_counters(self, wt, t):
        ...
        if "counters" in self.pb.pbase.results_to_write:
            ...
            if self.pb.comm.rank == 0:
                f = open(
                    self.pb.pbase.output_path + "/results_" + self.pb.pbase.simname + "_wt_ni_li_counters.txt",
                    mode,
                )
