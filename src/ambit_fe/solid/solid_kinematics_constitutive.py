...
class constitutive:
    ...
    def S(self, u_, v_, pp=None, ivar=None, returnquantity="stress"):
    ...
    def sigma(self, u_, v_, pp=None, ivar=None):
        return (1.0 / self.kin.J(u_)) * self.kin.F(u_) * self.S(u_, v_, pp=pp, ivar=ivar) * self.kin.F(u_).T

    # deviatoric part of Cauchy stress tensor: sigma_dev = sigma - tr(sigma)/3 I
    def sigma_dev(self, u_, v_, pp=None, ivar=None):
        return ufl.dev(self.sigma(u_, v_, pp=pp, ivar=ivar))

    # von Mises Cauchy stress
    def sigma_vonmises(self, u_, v_, pp=None, ivar=None):
        return ufl.sqrt(
            3.0
            * 0.5
            * ufl.inner(
                self.sigma_dev(u_, v_, pp=pp, ivar=ivar),
                self.sigma_dev(u_, v_, pp=pp, ivar=ivar),
            )
        )

    # 1st Piola-Kirchhoff stress tensor: P = F*S
    def P(self, u_, v_, pp=None, ivar=None):
        return self.kin.F(u_) * self.S(u_, v_, pp=pp, ivar=ivar)

    # Kirchhoff stress tensor: tau = J * sigma
    def tau_kirch(self, u_, v_, pp=None, ivar=None):
        return self.kin.J(u_) * self.sigma(u_, v_, pp=pp, ivar=ivar)

    # Mandel stress tensor: M = C*S
    def M(self, u_, v_, pp=None, ivar=None):
        return self.kin.C(u_) * self.S(u_, v_, pp=pp, ivar=ivar)

    # elastic 2nd Piola-Kirchhoff stress tensor
    def S_e(self, u_, v_, pp=None, ivar=None):
        theta_ = ivar["theta"]
        return self.F_g(theta_) * self.S(u_, v_, pp=pp, ivar=ivar) * self.F_g(theta_).T

    # elastic Mandel stress tensor: M = C*S
    def M_e(self, u_, v_, C_, pp=None, ivar=None):
        theta_ = ivar["theta"]
        return self.C_e(C_, theta_) * self.S_e(u_, v_, pp=pp, ivar=ivar)
    ...
    def res_dtheta_growth(self, u_, v_, pp, ivar, theta_old_, dt, thres, rquant):
        theta_ = ivar["theta"]
        ...
        if self.growth_trig == "volstress":
            trigger = reduc * ufl.tr(self.M_e(u_, v_, self.kin.C(u_), pp=pp, ivar=ivar))
    ...
    def dtheta_dC(self, u_, v_, pp, ivar, theta_old_, dt, thres):
        theta_ = ivar["theta"]

        dFg_dtheta = self.F_g(theta_, tang=True)

        ktheta = self.res_dtheta_growth(u_, v_, pp, ivar, theta_old_, dt, thres, "ktheta")
        K_growth = self.res_dtheta_growth(u_, v_, pp, ivar, theta_old_, dt, thres, "tang")

        i, j, k, l = ufl.indices(4)

        if self.growth_trig == "volstress":
            Cmat, _ = self.S(u_, v_, pp=pp, ivar=ivar, returnquantity="tangent")
            ...
            tangdC = (ktheta * dt / K_growth) * (
                self.S(u_, v_, pp=pp, ivar=ivar) + 0.5 * ufl.as_tensor(self.kin.C(u_)[i, j] * Cmat[i, j, k, l], (k, l))
            )
    ...
    def dS_dFg(self, u_, v_, pp, ivar, theta_old_, dt):
        theta_ = ivar["theta"]
        ...
        Fginv_outertop_S = ufl.as_tensor(
            ufl.inv(self.F_g(theta_))[i, k] * self.S(u_, v_, pp=pp, ivar=ivar)[j, l],
            (i, j, k, l),
        )
        S_outerbot_Fginv = ufl.as_tensor(
            self.S(u_, v_, pp=pp, ivar=ivar)[i, l] * ufl.inv(self.F_g(theta_))[j, k],
            (i, j, k, l),
        )
    ...
    def Cgrowth(self, u_, v_, pp, ivar, theta_old_, dt, thres):
        theta_ = ivar["theta"]
        ...
        dtheta_dC_ = self.dtheta_dC(u_, v_, pp, ivar, theta_old_, dt, thres)

        dS_dFg_ = self.dS_dFg(u_, v_, pp, ivar, theta_old_, dt)
    ...
    def dtheta_dp(self, u_, v_, pp, ivar, theta_old_, dt, thres):
        theta_ = ivar["theta"]
        ...
        if self.growth_trig == "volstress":
            tangdp = (ktheta * dt / K_growth) * (ufl.diff(ufl.tr(self.M_e(u_, v_, self.kin.C(u_), pp=pp, ivar=ivar)), pp["p"]))
    ...
    # has to be set analytically, since nonlinear Gauss point theta cannot be expressed as
    # function of u, so ufl cannot take care of it...
    def Cgrowth_p(self, u_, v_, pp, ivar, theta_old_, dt, thres):
        theta_ = ivar["theta"]
        ...
        dtheta_dp_ = self.dtheta_dp(u_, v_, pp, ivar, theta_old_, dt, thres)
    ...
    # has to be set analytically, since nonlinear Gauss point theta cannot be expressed as
    # function of u, so ufl cannot take care of it...
    def Cremod(self, u_, v_, pp, ivar, theta_old_, dt, thres):
        theta_ = ivar["theta"]
    ...
    # has to be set analytically, since nonlinear Gauss point theta cannot be expressed as
    # function of u, so ufl cannot take care of it...
    def Cremod_p(self, u_, v_, pp, ivar, theta_old_, dt, thres):
        theta_ = ivar["theta"]
