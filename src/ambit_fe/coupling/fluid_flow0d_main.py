...
class FluidmechanicsFlow0DProblem(problem_base):
    ...
    def set_problem_residual_jacobian_forms_coupling(self):
        ...
        for i in range(self.num_coupling_surf):
            ...
            self.dcq_form.append(
                fem.form(
                    self.cq_factor[i] * self.dcq[i],
                    entity_maps=self.pbf.io.entity_maps,
                )
            )
    ...
    def set_problem_vector_matrix_structures_coupling(self):
        ...
        for n in range(self.num_coupling_surf):
            self.dofs_coupling_vq[n] = meshutils.get_index_set_id(self.pbf.io, self.pbf.V_v, self.surface_vq_ids[n], self.pbf.io.mesh.topology.dim-1, self.comm)

            self.k_sv_subvec.append(self.k_sv_vec[n].getSubVector(self.dofs_coupling_vq[n]))

            sze_sv.append(self.k_sv_subvec[-1].getSize())

            self.dofs_coupling_p[n] = meshutils.get_index_set_id(self.pbf.io, self.pbf.V_v, self.surface_p_ids[n], self.pbf.io.mesh.topology.dim-1, self.comm)
        ...
        if self.condense_0d:
            ...
            self.K_lm_inv.setValuesBlocked(
                ...
                islm,
                np.ones(
                    (self.num_coupling_surf, self.num_coupling_surf),
                    dtype=np.int32,
                ),
                addv=PETSc.InsertMode.INSERT,
            )
    ...
    def assemble_residual_coupling(self, t, subsolver=None):
        ...
        self.pb0.auxdata["q"], self.pb0.auxdata["p"] = (
            self.pbf.qv_,
            self.pbf.pu_,
        )
    ...
    def assemble_stiffness_3d0d(self, t, LM_sq, subsolver=None, condensed_res_action=False):
        ...
        if self.condense_0d:
            if subsolver is not None:
                ...
                for i in range(ls, le):
                    for j in range(self.num_coupling_surf):
                        self.K_lm_inv.setValue(
                            i,
                            j,
                            self.K_lm_array_inv[i, j],
                            addv=PETSc.InsertMode.INSERT,
                        )
                self.K_lm_inv.assemble()
                ...
                if self.condense_0d_model == "diag":
                    for i in range(ls, le):
                        self.K_lm_invmat.setValue(
                            i,
                            i,
                            self.K_lm_array_invmat[i, i],
                            addv=PETSc.InsertMode.INSERT,
                        )
                    self.K_lm_invmat.assemble()
