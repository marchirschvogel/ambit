...
class FluidmechanicsMultiphaseProblem(problem_base):
    def __init__(
        ...
        self.var_names = self.pbf.var_names + self.pbp.var_names
        self.eq_names = self.pbf.eq_names + self.pbp.eq_names
    ...
    def set_variational_forms_residual_coupling(self):
        ...
        for n, M in enumerate(self.pbf.domain_ids):
            if self.capillary_force_from_korteweg_stress:
                ...
                self.capillary_force += self.pbf.vf.capillary_force(self.pbp.phi, self.pbp.mu, self.pbf.dx(M), F=self.pbf.alevar["Fale"])
