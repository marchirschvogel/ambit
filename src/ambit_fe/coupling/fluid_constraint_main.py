...
class FluidmechanicsConstraintProblem(problem_base):
    ...
    def set_variational_forms_residual_coupling(self):
        ...
        (
            self.power_coupling,
            self.power_coupling_old,
            self.power_coupling_mid,
        ) = (
            ufl.as_ufl(0),
            ufl.as_ufl(0),
            ufl.as_ufl(0),
        ...
        for n in range(self.num_coupling_surf):
            ...
            for i in range(len(self.surface_lm_ids[n])):
                ...
                if self.coupling_params["multiplier_physics"][n]["type"] == "pressure":
                    ...
                    df_back_ += self.pbf.vf.deltaW_ext_membrane(
                        self.pbf.ki.F(self.pbf.ufluid),
                        self.pbf.ki.Fdot(self.pbf.v),
                        None,
                        params_,
                    ...
                    df_mid_ += -self.pbf.vf.deltaW_ext_robin_valve_deriv_visc(
                        self.pbf.vel_mid,
                        ds_p,
                        fcts=fct_side,
                        w=self.pbf.alevar["w_mid"],
                    ...
                    df_back_ += -self.pbf.vf.deltaW_ext_robin_valve_deriv_visc(
                        self.pbf.v,
                        ds_p,
                        fcts=fct_side,
                        w=self.pbf.alevar["w"],
    ...
    def set_problem_residual_jacobian_forms_coupling(self):
        ...
        utilities.print_status(
            "FEM form compilation for fluid-constraint coupling...",
            self.comm,
            e=" ",
        )
        ...
        for i in range(self.num_coupling_surf):
            if self.on_subdomain[i]:
                ...
                em_u = {
                    self.pbf.mesh: self.pbf.io.submshes_emap[self.coupling_params["constraint_physics"][i]["domain"]][1]
                }
            else:
    ...
    def set_problem_vector_matrix_structures_coupling(self):
        ts = time.time()
        utilities.print_status("Creating vector and matrix data structures for fluid-constraint coupling...", self.pbase.comm, e=" ")
        ...
        for n in range(self.num_coupling_surf):
            ...
            self.dofs_coupling_p[n] = meshutils.get_index_set(self.pbf.V_v, self.comm, pb=self.pbf, identifier=self.surface_lm_ids[n], codim=self.pbf.io.mesh.topology.dim-1)
        ...
        self.K_vs = PETSc.Mat().createAIJ(
            size=(
                (locmatsize, matsize),
                (PETSc.DECIDE, self.num_coupling_surf),
            ),
            bsize=None,
    ...
    def read_restart(self, sname, N):
        ...
        if N > 0:
            restart_data = np.loadtxt(
                self.pbf.io.output_path + "/checkpoint_" + sname + "_lm_" + str(N) + ".txt",
                ndmin=1,
            )
