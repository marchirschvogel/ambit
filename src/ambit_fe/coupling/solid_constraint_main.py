...
class SolidmechanicsConstraintProblem(problem_base):
    ...
    def set_problem_residual_jacobian_forms_coupling(self):
        ...
        utilities.print_status(
            "FEM form compilation for solid-constraint coupling...",
            self.comm,
            e=" ",
        )
    ...
    def set_problem_vector_matrix_structures_coupling(self):
        ...
        for n in range(self.num_coupling_surf):
            self.dofs_coupling_vq[n] = meshutils.get_index_set(self.pbs.V_u, self.comm, pb=self.pbs, identifier=self.surface_vq_ids[n], codim=self.pbs.io.mesh.topology.dim-1)
            ...
            if self.coupling_params["multiplier_physics"][n]["type"] == "pressure":
                self.dofs_coupling_p[n] = meshutils.get_index_set(self.pbs.V_u, self.comm, pb=self.pbs, identifier=self.surface_lm_ids[n], codim=self.pbs.io.mesh.topology.dim-1)
            if self.coupling_params["multiplier_physics"][n]["type"] == "active_stress":
                self.dofs_coupling_p[n] = meshutils.get_index_set(self.pbs.V_u, self.comm, pb=self.pbs, identifier=self.surface_lm_ids[n], codim=self.pbs.io.mesh.topology.dim)
    ...
    def read_restart(self, sname, N):
        ...
        if N > 0:
            restart_data = np.loadtxt(
                self.pbs.io.output_path + "/checkpoint_" + sname + "_lm_" + str(N) + ".txt",
                ndmin=1,
            )
...
class SolidmechanicsConstraintSolver(solver_base):
    ...
    def solve_initial_state(self):
        ...
        if self.pb.pbs.timint != "static" and self.pb.pbase.restart_step == 0:
            ...
            self.solnln.solve_consistent_init(res_a, jac_aa, self.pb.pbs.a_old)
