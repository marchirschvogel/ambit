...
class FluidmechanicsAleFlow0DProblem(problem_base):
    def __init__(
        self,
        pbase,
        io_params,
        ...
        if self.pbfa.coupling_strategy == "monolithic":
            ...
            self.numdof = [
                self.pbf.numdof + self.pbf0.LM.getSize(),
                self.pba.numdof,
            ]
        ...
        self.eq_names = self.pbf.eq_names + ["3D0D coup constraint"] + self.pba.eq_names
    ...
    def set_problem_residual_jacobian_forms_coupling(self):
        if self.pbfa.coupling_strategy == "monolithic":
            ...
            for i in range(self.pbf0.num_coupling_surf):
                self.dcqd_form.append(
                    fem.form(
                        self.pbf0.cq_factor[i] * self.dcqd[i],
    ...
    def set_problem_vector_matrix_structures_coupling(self):
        if self.pbfa.coupling_strategy == "monolithic":
            ...
            self.K_sd = PETSc.Mat().createAIJ(
                size=(
                    (PETSc.DECIDE, self.pbf0.num_coupling_surf),
                    (locmatsize, matsize),
                ),
                bsize=None,
                nnz=max(sze_sd),
    ...
    def initialize_nonlinear_solver(self):
        ...
        if self.pb.pbfa.coupling_strategy == "monolithic":
            ...
            self.solnln = solver_nonlin.solver_nonlinear(
                [self.pb.pbf0, self.pb.pba],
                self.solver_params,
                subsolver=self.subsol,
                cp=self.pb,
            )
        else:
        ...
        if self.pb.pbfa.coupling_strategy == "monolithic":
            ...
            if self.pb.pbf0.rom:
                ...
                self.pb.pbf0.rom.set_reduced_data_structures_matrix(
                    self.pb.pbf0.K_list,
                    self.pb.pbf0.K_list_rom,
                    self.pb.pbf0.K_list_tmp,
                )
