...
class FluidmechanicsAleConstraintProblem(problem_base):
    def __init__(
        ...
        if self.pbfa.coupling_strategy == "monolithic":
            ...
            self.numdof = [
                self.pbf.numdof + self.pbfc.LM.getSize(),
                self.pba.numdof,
            ]
        ...
        self.var_names = self.pbf.var_names + ["LM"] + self.pba.var_names
        self.eq_names = self.pbf.eq_names + ["constraint"] + self.pba.eq_names
    ...
    def set_problem_vector_matrix_structures_coupling(self):
        if self.pbfa.coupling_strategy == "monolithic":
            ts = time.time()
            utilities.print_status("Creating vector and matrix data structures for ALE-constraint coupling...", self.pbase.comm, e=" ")
    ...
    def get_solver_index_sets(self, isoptions={}, blocked=False):
        ...
        if blocked:
            offset_v = vvec_or0 + self.pbf.p.x.petsc_vec.getOwnershipRange()[0] + self.pbfc.LM.getOwnershipRange()[0]
        else:
        ...
        if self.pb.pbfa.coupling_strategy == "monolithic":
            ...
            if self.pb.pbfc.rom:
                ...
                self.pb.pbfc.rom.set_reduced_data_structures_matrix(
                    self.pb.pbfc.K_list,
                    self.pb.pbfc.K_list_rom,
                    self.pb.pbfc.K_list_tmp,
                )
