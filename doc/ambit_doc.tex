...
\begin{align}
\delta \mathcal{W}_{\mathrm{int}}(\bs{u};\delta\bs{u}) &= \int\limits_{\Om_{0}} \bs{P}(\bs{u},\bs{v}(\bs{u})) : \nabla_{\bs{X}} \delta\bs{u} \,\mathrm{d}V \label{eq:deltaWint}
\end{align}
- external virtual work (conservative Neumann load):
\begin{align}
\delta \mathcal{W}_{\mathrm{ext}}(\delta\bs{u}) &= \int\limits_{\Gm_{0}^{\mathrm{N}}} \hat{\bs{t}}_{0} \cdot \delta\bs{u} \,\mathrm{d}A \label{eq:deltaWext_pk1}
\end{align}
...
\begin{align}
r_v(\bs{v},p;\delta\bs{v}) &:= \delta \mathcal{P}_{\mathrm{kin}}(\bs{v}) + \delta \mathcal{P}_{\mathrm{int}}(\bs{v},p) - \delta \mathcal{P}_{\mathrm{ext}}(\bs{v}) = 0, \quad \forall \; \delta\bs{v} \label{eq:res_v_fluid}\\
r_p(\bs{v};\delta p) &:= \delta \mathcal{P}_{\mathrm{pres}}(\bs{v}), \quad \forall \; \delta p \label{eq:res_p_fluid}
\end{align}
...
\int\limits_{\Om_t} \rho\,(\nabla\cdot\bs{v})\,\delta p\,\mathrm{d}v
\end{align}
- external virtual power (Neumann load):
\begin{align}
\delta \mathcal{P}_{\mathrm{ext}}(\delta\bs{v}) &= \int\limits_{\Gm_t^{\mathrm{N}}} \hat{\bs{t}} \cdot \delta\bs{v} \,\mathrm{d}a
\end{align}
...
\begin{align}
r_p \leftarrow r_p &+ \int\limits_{\Om_t} d_1\,((\bs{\nabla}\bs{v})\,\bs{v}) \cdot (\bs{\nabla}\delta p)\,\mathrm{d}v \\
&+ \int\limits_{\Om_t} d_3\,(\bs{\nabla}p) \cdot (\bs{\nabla}\delta p)\,\mathrm{d}v 
\end{align}
