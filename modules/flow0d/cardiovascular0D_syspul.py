...
class cardiovascular0Dsyspul(cardiovascular0Dbase):
    ...
    def equation_map(self):
        ...
        p_ati1_l_, p_ato1_l_ = sp.Symbol('p_ati1_l_'), sp.Symbol('p_ato1_l_')
        q_vout_l_          = sp.Symbol('q_vout_l_')
        p_vi1_l_, p_vo1_l_ = sp.Symbol('p_vi1_l_'), sp.Symbol('p_vo1_l_')
        ...
        p_ati1_r_, p_ato1_r_ = sp.Symbol('p_ati1_r_'), sp.Symbol('p_ato1_r_')
        q_vout_r_          = sp.Symbol('q_vout_r_')
        p_vi1_r_, p_vo1_r_ = sp.Symbol('p_vi1_r_'), sp.Symbol('p_vo1_r_')
        ...
        VQ_v_l_, p_vi1_l_, p_vo1_l_ = self.set_coupling_state('lv', {'vq' : VQ_v_l_, 'pi1' : p_vi1_l_, 'po1' : p_vo1_l_}, [E_v_l_])
        VQ_v_r_, p_vi1_r_, p_vo1_r_ = self.set_coupling_state('rv', {'vq' : VQ_v_r_, 'pi1' : p_vi1_r_, 'po1' : p_vo1_r_}, [E_v_r_])
        VQ_at_l_, p_ati1_l_, p_ato1_l_ = self.set_coupling_state('la', {'vq' : VQ_at_l_, 'pi1' : p_ati1_l_, 'po1' : p_ato1_l_}, [E_at_l_])
        VQ_at_r_, p_ati1_r_, p_ato1_r_ = self.set_coupling_state('ra', {'vq' : VQ_at_r_, 'pi1' : p_ati1_r_, 'po1' : p_ato1_r_}, [E_at_r_])

        # set valve resistances
        R_vin_l_, R_vin_r_, R_vout_l_, R_vout_r_ = self.set_valve_resistances(p_vi1_l_,p_vo1_l_,p_vi1_r_,p_vo1_r_,p_ato1_l_,p_ato1_r_,p_ar_sys_,p_ar_pul_)
        ...
        self.f_[1] = (p_ato1_l_-p_vi1_l_)/R_vin_l_ - q_vin_l_
        self.f_[2] = -q_vin_l_ + q_vout_l_ - (1-self.switch_V[0]) * VQ_v_l_
        self.f_[3] = (p_vo1_l_-p_ar_sys_)/R_vout_l_ - q_vout_l_
        ...
        self.f_[7] = (p_ati1_r_-p_ven_sys_)/self.R_ven_sys + q_ven_sys_
                # -----------------------------------------------------------
        self.f_[8] = -q_ven_sys_ + q_vin_r_ - (1-self.switch_V[3]) * VQ_at_r_
        self.f_[9] = (p_ato1_r_-p_vi1_r_)/R_vin_r_ - q_vin_r_
        self.f_[10] = -q_vin_r_ + q_vout_r_ - (1-self.switch_V[1]) * VQ_v_r_
        self.f_[11] = (p_vo1_r_-p_ar_pul_)/R_vout_r_ - q_vout_r_
        ...
        self.f_[15] = (p_ati1_l_-p_ven_pul_)/self.R_ven_pul + q_ven_pul_


        # auxiliary variable vector (mainly in order to store quantities for post-processing)
        self.a_[0]  = VQ_at_l_ * (1-self.switch_p[2]) + p_ati1_l_ * self.switch_p[2]
        self.a_[1]  = p_ato1_l_ * self.switch_p[2]
        self.a_[2]  = VQ_v_l_ * (1-self.switch_p[0]) + p_vi1_l_ * self.switch_p[0]
        self.a_[3]  = p_vo1_l_ * self.switch_p[0]
        ...
        self.a_[7]  = 0.
                # -----------------------------------------------------------
        self.a_[8]  = VQ_at_r_ * (1-self.switch_p[3]) + p_ati1_r_ * self.switch_p[3]
        self.a_[9]  = p_ato1_r_ * self.switch_p[3]
        self.a_[10] = VQ_v_r_ * (1-self.switch_p[1]) + p_vi1_r_ * self.switch_p[1]
        self.a_[11] = p_vo1_r_ * self.switch_p[1]
...
# syspul model with separate venous branches (4 to LA, 2 to RA)

class cardiovascular0Dsyspul_veins(cardiovascular0Dsyspul):
    ...
    def setup_arrays(self):
        ...
        self.vindex_ch = [3,12,1,10] # coupling variable indices (decreased by 1 for pressure coupling!)
        self.vname_prfx, self.cname_prfx = ['p']*4, ['V']*4 # default values

        # set those ids which are relevant for monolithic direct coupling
        self.v_ids, self.c_ids = [], []
        self.cindex_ch = [2,11,0,9]
    ...
    def equation_map(self):
        ...
        self.varmap={'q_vin_l' : 0+self.si[2], ''+self.vname_prfx[2]+'_at_l' : 1-self.si[2], 'q_vout_l' : 2+self.si[0], ''+self.vname_prfx[0]+'_v_l' : 3-self.si[0], 'p_ar_sys' : 4, 'q_ar_sys' : 5, 'p_ven_sys' : 6, 'q_ven1_sys' : 7, 'q_ven2_sys' : 8, 'q_vin_r' : 9+self.si[3], ''+self.vname_prfx[3]+'_at_r' : 10-self.si[3], 'q_vout_r' : 11+self.si[1], ''+self.vname_prfx[1]+'_v_r' : 12-self.si[1], 'p_ar_pul' : 13, 'q_ar_pul' : 14, 'p_ven_pul' : 15, 'q_ven1_pul' : 16, 'q_ven2_pul' : 17, 'q_ven3_pul' : 18, 'q_ven4_pul' : 19}
        self.auxmap={''+self.cname_prfx[2]+'_at_l' : 0, ''+self.cname_prfx[0]+'_v_l' : 2, 'V_ar_sys' : 4, 'V_ven_sys' : 6, ''+self.cname_prfx[3]+'_at_r' : 9, ''+self.cname_prfx[1]+'_v_r' : 11, 'V_ar_pul' : 13, 'V_ven_pul' : 15}
        ...
        self.t_            = sp.Symbol('t_')
        q_vin_l_           = sp.Symbol('q_vin_l_')
        for n in range(vp): p_ati_l_.append(sp.Symbol('p_ati'+str(n+1)+'_l_'))
        p_ato1_l_          = sp.Symbol('p_ato1_l_')
        q_vout_l_          = sp.Symbol('q_vout_l_')
        p_vi1_l_, p_vo1_l_ = sp.Symbol('p_vi1_l_'), sp.Symbol('p_vo1_l_')
        p_ar_sys_          = sp.Symbol('p_ar_sys_')
        q_ar_sys_          = sp.Symbol('q_ar_sys_')
        p_ven_sys_         = sp.Symbol('p_ven_sys_')
        for n in range(vs): q_ven_sys_.append(sp.Symbol('q_ven'+str(n+1)+'_sys_'))
        q_vin_r_           = sp.Symbol('q_vin_r_')
        for n in range(vs): p_ati_r_.append(sp.Symbol('p_ati'+str(n+1)+'_r_'))
        p_ato1_r_          = sp.Symbol('p_ato1_r_')
        q_vout_r_          = sp.Symbol('q_vout_r_')
        p_vi1_r_, p_vo1_r_ = sp.Symbol('p_vi1_r_'), sp.Symbol('p_vo1_r_')
        p_ar_pul_          = sp.Symbol('p_ar_pul_')
        q_ar_pul_          = sp.Symbol('q_ar_pul_')
        p_ven_pul_         = sp.Symbol('p_ven_pul_')
        for n in range(vp): q_ven_pul_.append(sp.Symbol('q_ven'+str(n+1)+'_pul_'))
        VQ_v_l_            = sp.Symbol('VQ_v_l_')
        VQ_v_r_            = sp.Symbol('VQ_v_r_')
        VQ_at_l_           = sp.Symbol('VQ_at_l_')
        VQ_at_r_           = sp.Symbol('VQ_at_r_')
        ...
        # dofs to differentiate w.r.t.
        self.x_[0+self.si[2]] = q_vin_l_
        self.x_[1-self.si[2]] = p_ati_l_[0]
        self.x_[2+self.si[0]] = q_vout_l_
        self.x_[3-self.si[0]] = p_vi1_l_
        self.x_[4] = p_ar_sys_
        self.x_[5] = q_ar_sys_
        ...
        self.x_[7+vs+self.si[3]] = q_vin_r_
        self.x_[8+vs-self.si[3]] = p_ati_r_[0]
        self.x_[9+vs+self.si[1]] = q_vout_r_
        self.x_[10+vs-self.si[1]] = p_vi1_r_
        self.x_[11+vs] = p_ar_pul_
        self.x_[12+vs] = q_ar_pul_
        self.x_[13+vs] = p_ven_pul_
        for n in range(vp):
            self.x_[14+vs+n] = q_ven_pul_[n]


        # TODO: FIX THIS for generalized number of veins!!!
        #chdict_la = {'vq' : VQ_at_l_, 'po1' : p_ato1_l_}
        #for n in range(vp): chdict_la['pi'+str(n+1)+''] = p_ati_l_[n]
        #chdict_ra = {'vq' : VQ_at_r_, 'po1' : p_ato1_r_}
        #for n in range(vs): chdict_ra['pi'+str(n+1)+''] = p_ati_r_[n]

        # set chamber states and variables (e.g., express V in terms of p and E in case of elastance models, ...)
        VQ_v_l_, p_vi1_l_, p_vo1_l_ = self.set_coupling_state('lv', {'vq' : VQ_v_l_, 'pi1' : p_vi1_l_, 'po1' : p_vo1_l_}, [E_v_l_])
        VQ_v_r_, p_vi1_r_, p_vo1_r_ = self.set_coupling_state('rv', {'vq' : VQ_v_r_, 'pi1' : p_vi1_r_, 'po1' : p_vo1_r_}, [E_v_r_])
        VQ_at_l_, p_ati_l_[0], p_ati_l_[1], p_ati_l_[2], p_ati_l_[3], p_ato1_l_ = self.set_coupling_state('la', {'vq' : VQ_at_l_, 'pi1' : p_ati_l_[0], 'pi2' : p_ati_l_[1], 'pi3' : p_ati_l_[2], 'pi4' : p_ati_l_[3], 'po1' : p_ato1_l_}, [E_at_l_])
        VQ_at_r_, p_ati_r_[0], p_ati_r_[1], p_ato1_r_ = self.set_coupling_state('ra', {'vq' : VQ_at_r_, 'pi1' : p_ati_r_[0], 'pi2' : p_ati_r_[1], 'po1' : p_ato1_r_}, [E_at_r_])
        ...
        for n in range(vs):
            R_ven_sys.append(vs*self.R_ven_sys)
            L_ven_sys.append(vs*self.L_ven_sys)

        for n in range(vp):
            R_ven_pul.append(vp*self.R_ven_pul)
            L_ven_pul.append(vp*self.L_ven_pul)
        ...
        for n in range(vs):
            self.df_[7+n] = (L_ven_sys[n]/R_ven_sys[n]) * q_ven_sys_[n]
                # -----------------------------------------------------------
        self.df_[7+vs] = VQ_at_r_ * self.switch_V[3]
        self.df_[8+vs] = 0.
        self.df_[9+vs] = VQ_v_r_ * self.switch_V[1]
        self.df_[10+vs] = 0.
        self.df_[11+vs] = self.C_ar_pul * (p_ar_pul_ - self.Z_ar_pul * q_vout_r_)
        self.df_[12+vs] = (self.L_ar_pul/self.R_ar_pul) * q_ar_pul_
        self.df_[13+vs] = self.C_ven_pul * p_ven_pul_
        for n in range(vp):
            self.df_[14+vs+n] = (L_ven_pul[n]/R_ven_pul[n]) * q_ven_pul_[n]
        ...
        for n in range(vs):
            self.f_[7+n] = (p_ati_r_[n]-p_ven_sys_)/R_ven_sys[n] + q_ven_sys_[n]
                # -----------------------------------------------------------
        self.f_[7+vs] = -sum(q_ven_sys_) + q_vin_r_ - (1-self.switch_V[3]) * VQ_at_r_
        self.f_[8+vs] = (p_ato1_r_-p_vi1_r_)/R_vin_r_ - q_vin_r_
        self.f_[9+vs] = -q_vin_r_ + q_vout_r_ - (1-self.switch_V[1]) * VQ_v_r_
        self.f_[10+vs] = (p_vo1_r_-p_ar_pul_)/R_vout_r_ - q_vout_r_
        self.f_[11+vs] = -q_vout_r_ + q_ar_pul_
        self.f_[12+vs] = (p_ven_pul_ - p_ar_pul_ + self.Z_ar_pul * q_vout_r_)/self.R_ar_pul + q_ar_pul_
        self.f_[13+vs] = -q_ar_pul_ + sum(q_ven_pul_)
        for n in range(vp):
            self.f_[14+vs+n] = (p_ati_l_[n]-p_ven_pul_)/R_ven_pul[n] + q_ven_pul_[n]


        # auxiliary variable vector (mainly in order to store quantities for post-processing)
        self.a_[0]  = VQ_at_l_ * (1-self.switch_p[2]) + p_ati_l_[0] * self.switch_p[2]
        self.a_[1]  = p_ato1_l_ * self.switch_p[2]
        self.a_[2]  = VQ_v_l_ * (1-self.switch_p[0]) + p_vi1_l_ * self.switch_p[0]
        self.a_[3]  = p_vo1_l_ * self.switch_p[0]
        self.a_[4]  = self.C_ar_sys * (p_ar_sys_ - self.Z_ar_sys * q_vout_l_) + self.V_ar_sys_u
        ...
        for n in range(vs):
            self.a_[7+n]  = 0.
                # -----------------------------------------------------------
        self.a_[7+vs]  = VQ_at_r_ * (1-self.switch_p[3]) + p_ati_r_[0] * self.switch_p[3]
        self.a_[8+vs] = p_ato1_r_ * self.switch_p[3]
        self.a_[9+vs] = VQ_v_r_ * (1-self.switch_p[1]) + p_vi1_r_ * self.switch_p[1]
        self.a_[10+vs] = p_vo1_r_ * self.switch_p[1]
        self.a_[11+vs] = self.C_ar_pul * (p_ar_pul_ - self.Z_ar_pul * q_vout_r_) + self.V_ar_pul_u
        self.a_[12+vs] = 0.
        self.a_[13+vs] = self.C_ven_pul * p_ven_pul_ + self.V_ven_pul_u
        for n in range(vp):
            self.a_[14+vs+n] = 0.
    ...
    def initialize(self, var, iniparam):
        ...
        var[6] = iniparam['p_ven_sys_0']
        for n in range(vs):
            var[7+n] = iniparam['q_ven'+str(n+1)+'_sys_0']
        var[7+vs+self.si[3]] = iniparam['q_vin_r_0']
        var[8+vs-self.si[3]] = iniparam[''+self.vname_prfx[3]+'_at_r_0']
        var[9+vs+self.si[1]] = iniparam['q_vout_r_0']
        var[10+vs-self.si[1]] = iniparam[''+self.vname_prfx[1]+'_v_r_0']
        var[11+vs] = iniparam['p_ar_pul_0']
        var[12+vs] = iniparam['q_ar_pul_0']
        var[13+vs] = iniparam['p_ven_pul_0']
        for n in range(vp):
            var[14+vs] = iniparam['q_ven'+str(n+1)+'_pul_0']
...
def postprocess_groups_syspul_veins(groups, indpertaftercyl=0, multiscalegandr=False):
