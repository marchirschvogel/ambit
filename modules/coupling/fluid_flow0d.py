    ...
    def set_variational_forms_and_jacobians(self):
        ...
        for n in range(self.num_coupling_surf):
            ...
            for i in range(len(self.surface_p_ids[n])):
                ...
                df_ += self.pbf.timefac*self.pbf.vf.surface(ds_p, Fale=self.pbf.alevar['Fale'])
            
                # add to fluid rhs contributions
                self.power_coupling += self.pbf.vf.deltaW_ext_neumann_normal(self.coupfuncs[-1], ds_p, Fale=self.pbf.alevar['Fale'])
                self.power_coupling_old += self.pbf.vf.deltaW_ext_neumann_normal(self.coupfuncs_old[-1], ds_p, Fale=self.pbf.alevar['Fale_old'])
...
class FluidmechanicsFlow0DSolver(solver_base):
    ...
    def print_timestep_info(self, N, t, wt):
        ...
        self.pb.pb0.ti.print_timestep(N, t, self.solnln.sepstring, self.pb.pbf.numstep, wt=wt)
