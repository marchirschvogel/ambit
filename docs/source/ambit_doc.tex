...
\textbf{Displacement-based}\\
Find $\bs{u}\in\boldsymbol{\mathcal{V}}_{u}^{h,D}$ such that
\begin{equation}
\label{equation-solid-weak-form}
\begin{aligned}
r(\bs{u};\delta\bs{u}) := \delta \mathcal{W}_{\mathrm{kin}}(\bs{u};\delta\bs{u}) + \delta \mathcal{W}_{\mathrm{int}}(\bs{u};\delta\bs{u}) - \delta \mathcal{W}_{\mathrm{ext}}(\bs{u};\delta\bs{u}) = 0, \end{aligned}
\end{equation}
for all $\delta\bs{u}\in\boldsymbol{\mathcal{V}}_{u}^{h}$\\
...
Find $(\bs{u},p) \in \boldsymbol{\mathcal{V}}_{u}^{h,D} \times \mathcal{V}_{p}^{h,D}$ such that
\begin{equation}
\label{equation-solid-weak-form-inc}
\begin{aligned}
r_u(\bs{u},p;\delta\bs{u}) &:= \delta \mathcal{W}_{\mathrm{kin}}(\bs{u};\delta\bs{u}) + \delta \mathcal{W}_{\mathrm{int}}(\bs{u},p;\delta\bs{u}) - \delta \mathcal{W}_{\mathrm{ext}}(\bs{u};\delta\bs{u}) = 0, \\
r_p(\bs{u};\delta p) &:= \delta \mathcal{W}_{\mathrm{pres}}(\bs{u};\delta p) = 0,
\end{aligned}
\end{equation}
for all $(\delta\bs{u},\delta p) \in \boldsymbol{\mathcal{V}}_{u}^{h} \times \mathcal{V}_{p}^{h}$\\
...
\begin{aligned}
\rho\left(\frac{\partial\bs{v}}{\partial t} + (\nabla\bs{v})\,\bs{v}\right) &= \nabla \cdot \bs{\sigma}(\bs{v},p) + \hat{\bs{b}} &&\text{in} \; \mathit{\Om}_t \times [0, T], \\
...
\begin{align}
\bs{\sigma} = -p \bs{I} + \eta \left(\nabla \bs{v} + (\nabla \bs{v})^{\mathrm{T}}\right)
\end{align}
where $\eta$ is the dynamic viscosity

\textbf{Weak Form}\\
Find $(\bs{v},p) \in \boldsymbol{\mathcal{V}}_{v}^{h,D} \times \mathcal{V}_{p}^{h,D}$ such that
\begin{equation}
\label{equation-fluid-weak-form}
\begin{aligned}
r_v(\bs{v},p;\delta\bs{v}) &:= \delta \mathcal{P}_{\mathrm{kin}}(\bs{v};\delta\bs{v}) + \delta \mathcal{P}_{\mathrm{int}}(\bs{v},p;\delta\bs{v}) - \delta \mathcal{P}_{\mathrm{ext}}(\bs{v};\delta\bs{v}) = 0, \quad \\
r_p(\bs{v};\delta p) &:= \delta \mathcal{P}_{\mathrm{pres}}(\bs{v};\delta p) = 0,
\end{aligned}
\end{equation}
for all $(\delta\bs{v},\delta p) \in \boldsymbol{\mathcal{V}}_{v}^{h} \times \mathcal{V}_{p}^{h}$
...
\begin{aligned}
\widehat{J}\rho\left(\left.\frac{\partial\bs{v}}{\partial t}\right|_{\bs{x}_{0}} + (\nabla_0\bs{v}\,\widehat{\bs{F}}^{-1})\,(\bs{v}-\widehat{\bs{w}})\right) &= \nabla_{0} \cdot\left(\widehat{J}\bs{\sigma} \widehat{\bs{F}}^{-\mathrm{T}}\right) + \widehat{J}\hat{\bs{b}} &&\text{in} \; \mathit{\Om}_0 \times [0, T],\\
\nabla_{0}\cdot\left(\widehat{J}\widehat{\bs{F}}^{-1}\bs{v}\right) &= 0 &&\text{in} \; \mathit{\Om}_0 \times [0, T],\\
...
with a Newtonian fluid constitutive law
\begin{align}
\bs{\sigma} = -p \bs{I} + \mu \left(\nabla_0 \bs{v}\,\widehat{\bs{F}}^{-1} + \widehat{\bs{F}}^{-\mathrm{T}}(\nabla_0 \bs{v})^{\mathrm{T}}\right)
\end{align}

\textbf{Weak form (ALE)}\\
Find $(\bs{v},p,\bs{d}) \in \boldsymbol{\mathcal{V}}_{v}^{h,D} \times \mathcal{V}_{p}^{h,D} \times \boldsymbol{\mathcal{V}}_{d}^{h,D}$ such that
\begin{equation}
\label{equation-fluid-ale-weak-form}
\begin{aligned}
r_v(\bs{v},p,\bs{d};\delta\bs{v}) &:= \delta \mathcal{P}_{\mathrm{kin}}(\bs{v},\bs{d};\delta\bs{v}) + \delta \mathcal{P}_{\mathrm{int}}(\bs{v},p,\bs{d};\delta\bs{v}) - \delta \mathcal{P}_{\mathrm{ext}}(\bs{v},\bs{d};\delta\bs{v}) = 0, \\
r_p(\bs{v},\bs{d};\delta p) &:= \delta \mathcal{P}_{\mathrm{pres}}(\bs{v},\bs{d};\delta p) = 0, \\
r_{d}(\bs{d};\delta\bs{d}) &= 0,
\end{aligned}
\end{equation}
for all $(\delta\bs{v},\delta p,\delta\bs{d}) \in \boldsymbol{\mathcal{V}}_{v}^{h} \times \mathcal{V}_{p}^{h} \times \boldsymbol{\mathcal{V}}_{d}^{h}$
...
\int\limits_{\Om_0} \widehat{J}\bs{\sigma}\widehat{\bs{F}}^{-\mathrm{T}} : \nabla_{0} \delta\bs{v} \,\mathrm{d}V
\end{aligned}
\end{equation}
...
\int\limits_{\Om_0} \nabla_{0}\cdot\left(\widehat{J}\widehat{\bs{F}}^{-1}\bs{v}\right) \delta p\,\mathrm{d}V
\end{aligned}
\end{equation}
...
& \qquad\quad \cdot\left(\widehat{J}\rho\left(\left.\frac{\partial \bs{v}}{\partial t}\right|_{\bs{x}_{0}} + (\nabla_0\bs{v}\,\widehat{\bs{F}}^{-1})\,(\bs{v}-\widehat{\bs{w}})\right) - \nabla_{0}\cdot\left(\widehat{J}\bs{\sigma}\widehat{\bs{F}}^{-\mathrm{T}}\right)\right)\,\mathrm{d}V \\
...
\begin{align}
\bs{\sigma} = -p \bs{I} + \eta \left(\nabla \bs{v} + (\nabla \bs{v})^{\mathrm{T}}\right) + \left(\zeta - \frac{2}{d}\eta\right)\nabla\cdot\bs{v}
\end{align}
...
\phi(\bs{x},0) &= \hat{\phi}_{0}(\bs{x}) &&\text{in} \; \mathit{\Om}_t,
\end{aligned}
\end{equation}
...
Find $(\bs{v},p,\phi,\mu) \in \boldsymbol{\mathcal{V}}_{v}^{h,D} \times \mathcal{V}_{p}^{h,D} \times \mathcal{V}_{\phi}^{h,D} \times \mathcal{V}_{\mu}^{h,D}$ such that
\begin{equation}
\begin{aligned}
...
for all $(\delta\bs{v},\delta p,\delta\phi,\delta\mu) \in \boldsymbol{\mathcal{V}}_{v}^{h} \times \mathcal{V}_{p}^{h} \times \mathcal{V}_{\phi}^{h} \times \mathcal{V}_{\mu}^{h}$, with
\begin{align}
...
\begin{aligned}
\ROP_{n+1} = \begin{bmatrix} \ROP_{v}(\bs{\mathsf{v}},\bs{\mathsf{p}},\bs{\upphi},\bs{\upmu}) \\ \ROP_{p}(\bs{\mathsf{p}},\bs{\mathsf{v}},\bs{\upphi}) \\ \ROP_{\phi}(\bs{\upphi},\bs{\mathsf{v}},\bs{\mathsf{p}},\bs{\upmu}) \\ \ROP_{\mu}(\bs{\upmu},\bs{\upphi}) \end{bmatrix}_{n+1} = \bs{\mathsf{0}}
\end{aligned}
\end{equation}
...
\begin{aligned}
\begin{bmatrix} \bs{\mathsf{K}}_{vv} & \bs{\mathsf{K}}_{vp} & \bs{\mathsf{K}}_{v\phi} & \bs{\mathsf{K}}_{v\mu} \\ \\ \bs{\mathsf{K}}_{pv} & \bs{\mathsf{K}}_{pp} & \bs{\mathsf{K}}_{p\phi} & \zerom \\ \\ \bs{\mathsf{K}}_{\phi v} & \bs{\mathsf{K}}_{\phi p} & \bs{\mathsf{K}}_{\phi \phi} & \bs{\mathsf{K}}_{\phi \mu} \\ \\ \zerom & \zerom & \bs{\mathsf{K}}_{\mu\phi} & \bs{\mathsf{K}}_{\mu\mu} \end{bmatrix}_{n+1}^{k}\begin{bmatrix} \Delta\bs{\mathsf{v}} \\ \\ \Delta\bs{\mathsf{p}} \\ \\ \Delta\bs{\upphi} \\ \\ \Delta\bs{\upmu}\end{bmatrix}_{n+1}^{k+1}=-\begin{bmatrix} \ROP_{v} \\ \\ \ROP_{p} \\ \\ \ROP_{\phi} \\ \\ \ROP_{\mu}\end{bmatrix}_{n+1}^{k}
\end{aligned}
\end{equation}
