...
\begin{align}
\bs{\sigma} = -p \bs{I} + 2 \eta\,\bs{\gamma} = -p \bs{I} + \eta \left(\nabla \bs{v} + (\nabla \bs{v})^{\mathrm{T}}\right)
\end{align}
...
\mu - \frac{\mathrm{d}f}{\mathrm{d}\phi} &= -\nabla\cdot(\kappa \nabla \phi) &&\text{in} \; \mathit{\Om}_t \times [0, T],\\
...
\subsection{Demo: Multiphase fluid}\label{demo-multiphase}
